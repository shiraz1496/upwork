    ...
    const result = accounts.map((account) => {
      ...
      const snapshotSummaries = snapshots
        .map((s) => {
          ...
          const interviewed =
            (s.proposalsInterviewedBoosted ?? 0) + (s.proposalsInterviewedOrganic ?? 0);
          ...
          return {
            ...
            capturedBy: s.capturedByUser
              ? { id: s.capturedByUser.id, name: s.capturedByUser.name }
              : null,
          };
        })
        .reverse();

      const latestSent = latest
        ? (latest.proposalsSentBoosted ?? 0) + (latest.proposalsSentOrganic ?? 0)
        : 0;
      const latestViewed = latest
        ? (latest.proposalsViewedBoosted ?? 0) + (latest.proposalsViewedOrganic ?? 0)
        : 0;
      const latestInterviewed = latest
        ? (latest.proposalsInterviewedBoosted ?? 0) + (latest.proposalsInterviewedOrganic ?? 0)
        : 0;
      const latestHired = latest
        ? (latest.proposalsHiredBoosted ?? 0) + (latest.proposalsHiredOrganic ?? 0)
        : 0;
      ...
      return {
        ...
        latestSnapshot: latest
          ? {
              ...
              interviewRate:
                latestSent > 0 ? Math.round((latestInterviewed / latestSent) * 1000) / 10 : 0,
        ...
        jobs: account.jobs.map((j) => ({
          ...
          clientPhoneVerified: j.clientPhoneVerified,
          clientRating: j.clientRating,
          clientReviewScore: j.clientReviewScore,
          clientReviews: j.clientReviews,
          clientSpent: j.clientSpent,
          clientCountry: j.clientCountry,
          clientCity: j.clientCity,
          clientHires: j.clientHires,
          clientActiveHires: j.clientActiveHires,
          clientHireRate: j.clientHireRate,
          clientOpenJobs: j.clientOpenJobs,
          clientJobsPosted: j.clientJobsPosted,
          clientMemberSince: j.clientMemberSince,
          clientIndustry: j.clientIndustry,
          clientCompanySize: j.clientCompanySize,
