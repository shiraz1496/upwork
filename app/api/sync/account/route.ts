...
export const POST = withAttribution(async ({ req, member }) => {
  try {
    ...
    const isRealName =
      (name && !/^\w{10,}$/.test(name) && /[a-zA-Z].*\s/.test(name)) ||
      (name && name.length < 30 && /[A-Z]/.test(name) && !/^\d+$/.test(name));

    const account = await prisma.account.upsert({
      where: { freelancerId: String(freelancerId) },
      update: {
        ...(isRealName ? { name } : {}),
        ...(jss != null ? { jss: Number(jss) } : {}),
        ...(connectsBalance != null ? { connectsBalance: Number(connectsBalance) } : {}),
      },
      create: {
